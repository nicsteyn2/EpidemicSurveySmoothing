...
# Samples from the full marginal posterior distribution of the hidden-states given a bootstrap filter and set of parameter samples
# Returns an NxT matrix of samples from the marginal posterior distribution of the hidden states, 
# Also returns an NxK matrix of parameter samples correpsonding to the hidden state samples
function marginalPosterior(bootstrapFilter::Function, θ, Y::DataFrame, opts::Dict; showProgress=false)
    ...
    Threads.@threads for ii = 2:nPosteriorParamSamples
        try
            ...
            println("Error at $ii^th parameter sample $(θsample[ii,:]).")
            rethrow(e)
        end
        next!(ProgBar)
    end
    ...
    return(X, repeat(θsample, inner=(N, 1)))
