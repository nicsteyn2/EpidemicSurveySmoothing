...
df_weighted = rbind(
  read.csv("paper/outputs/s9c-react-weighted/steyn_r1to7_states.csv") %>% mutate(Method="Weighted", rounds="1-to-7"),
  read.csv("paper/outputs/s9c-react-weighted/steyn_r8to13_states.csv") %>% mutate(Method="Weighted", rounds="8-to-13"),
  read.csv("paper/outputs/s9c-react-weighted/steyn_r14to16_states.csv") %>% mutate(Method="Weighted", rounds="14-to-16"),
  read.csv("paper/outputs/s9c-react-weighted/steyn_r17to19_states.csv") %>% mutate(Method="Weighted", rounds="17-to-19")
)

df = rbind(df_overdisp, df_weighted) %>%
...
plt_rt = ggplot(df %>% filter(variable=="rt")) +
  ...
  scale_color_manual(values=c("#2d96b3", "#6aa84f")) +
  scale_fill_manual(values=c("#2d96b3", "#6aa84f")) +
...
plt
ggsave("paper/figures/s9c-react-weighted.png", plt, width=24, height=16, units="cm", dpi=600)
