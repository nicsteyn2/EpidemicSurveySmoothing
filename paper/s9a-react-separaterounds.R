...
# Load estimates
df = rbind(
  read.csv("paper/outputs/3-react/steyn_r1to7_states.csv") %>% mutate(Method="Separate", rounds="1-to-7"),
  read.csv("paper/outputs/3-react/steyn_r8to13_states.csv") %>% mutate(Method="Separate", rounds="8-to-13"),
  read.csv("paper/outputs/3-react/steyn_r14to16_states.csv") %>% mutate(Method="Separate", rounds="14-to-16"),
  read.csv("paper/outputs/3-react/steyn_r17to19_states.csv") %>% mutate(Method="Separate", rounds="17-to-19"),
  read.csv("paper/outputs/3-react/steyn_r1to19_states.csv") %>% mutate(Method="Entire study", rounds="1-to-19")
...
plt_rt = ggplot(df %>% filter(variable=="rt")) +
  ...
  scale_color_manual(values=c("#2d96b3", "darkred")) +
  scale_fill_manual(values=c("#2d96b3", "darkred")) +
...
plt_forlegend = plt_nPos + theme(legend.position="right") + labs(color="Period", fill="Period")
...
plt
ggsave("paper/figures/s9a-react-separaterounds.png", plt, width=24, height=16, units="cm", dpi=600)
