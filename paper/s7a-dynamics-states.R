...
plt_cov = ggplot(df_plt %>% filter(variable=="rt"), aes(x=nSamples, color=model, linetype=factor(sigma))) +
...
plt_width = ggplot(df_plt %>% filter(variable=="rt"), aes(x=nSamples, color=model, linetype=factor(sigma))) +
...
plt_forlegend = plt_cov + theme(legend.position="right") + labs(color="Model") + guides(linetype = "none")
...
plt
ggsave("paper/figures/s7a-dynamics-rt.png", plt, width=24, height=14, units="cm", dpi=600)
