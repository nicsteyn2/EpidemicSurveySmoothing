...
# Fit the model
st = time()
(df_states, df_params, θ, diag) = fitModel(WeightedModel, Y, opts; skipResamplingPMMH=true, verbose=true, posteriorPredictive="weighted", checkLogLikStdDev=false)
en = time()

# Save outputs
CSV.write("paper/outputs/s9c-react-weighted/steyn_r$(start_round)to$(end_round)_states.csv", df_states)
CSV.write("paper/outputs/s9c-react-weighted/steyn_r$(start_round)to$(end_round)_params.csv", df_params)

# Calculate nPos coverage
l = df_states.lower[df_states.variable .== "phat"]
u = df_states.upper[df_states.variable .== "phat"]
inPredInterval = (Y.phat .>= l) .& (Y.phat .<= u)
...
# Save model summary
outfile = open("paper/outputs/s9c-react-weighted/steyn_r$(start_round)to$(end_round)_summary.txt", "w")
...
write(outfile, "c = $(round(df_params.m[2], sigdigits=2)) ($(round(df_params.l[2], sigdigits=2)), $(round(df_params.u[2], sigdigits=2)))\n\n")
