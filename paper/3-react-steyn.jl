...
# (sd, ll) = estimateStdDevLogLik(100, BetaBinomialModel, [0.01, 2e-4], Y, opts)

# Fit the model
st = time()
(df_states, df_params, θ, diag) = fitModel(BetaBinomialModel, Y, opts; skipResamplingPMMH=true, verbose=true, posteriorPredictive="betabinom", checkLogLikStdDev=false)
en = time()

# Save outputs
CSV.write("paper/outputs/3-react/steyn_r$(start_round)to$(end_round)_states.csv", df_states)
CSV.write("paper/outputs/3-react/steyn_r$(start_round)to$(end_round)_params.csv", df_params)
...
# Save model summary
outfile = open("paper/outputs/3-react/steyn_r$(start_round)to$(end_round)_summary.txt", "w")
